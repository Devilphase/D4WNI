...
FiddlesticksMenu.Combo:Slider("EMana", "E if Mana % >", 30, 0, 80, 1)
...
FiddlesticksMenu.Harass:Boolean("E", "Use E", true)
...
FiddlesticksMenu.Killsteal:Boolean("W", "Killsteal with W", true)
...
FiddlesticksMenu.LaneClear:Boolean("E", "Use E", true)
...
FiddlesticksMenu.JungleClear:Boolean("E", "Use E", true)
...
if FiddlesticksMenu.Drawings.E:Value() then DrawCircle(GetOrigin(myHero),1075,1,25,GoS.Pink) end
end)

IOW:AddCallback(AFTER_ATTACK, function(target, mode)
  if IsReady(_E) then 
    if mode == "Combo" and target ~= nil and FiddlesticksMenu.Combo.E:Value() then
    CastSpell(_E)
    end
...
OnTick(function(myHero)
	...
	if IsReady(_R) and ValidTarget(target, 600) and FiddlesticksMenu.Combo.R:Value() and GetPercentMP(myHero) >= FiddlesticksMenu.Combo.RMana:Value() and EnemiesAround(GetOrigin(myHero), 600) >= FiddlesticksMenu.Combo.Rmin:Value() then
	CastSpell(_R)
        end
